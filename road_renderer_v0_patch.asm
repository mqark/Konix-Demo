; ===========================================================================
; KONIX MULTISYSTEM - ROAD RENDERER V0
; March 2026
;
; Static pseudo-3D road renderer prototype for a 256x200 8bpp display.
; Draws sky, ground, 35 road bands, and simple dashed lane stripes.
; Built directly on the proven blitter rectangle fill demo.
;
; ---------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------
; ROAD RENDERER CONSTANTS
; ---------------------------------------------------------------------------
ROAD_HORIZON_Y  equ 60
ROAD_BAND_H     equ 4
ROAD_NUM_BANDS  equ 35
ROAD_START_BAND equ 0
...
; ---------------------------------------------------------------------------
; SUBROUTINE: draw_fullwidth_rect
; ---------------------------------------------------------------------------
; Draw a full-width 256-pixel rectangle by issuing two 128-pixel rect fills.
; Caller sets: rect_y, rect_h, rect_c
...
; ---------------------------------------------------------------------------
; SUBROUTINE: draw_road_bands
; ---------------------------------------------------------------------------
; Draw the 35 horizontal road slices that create the pseudo-3D perspective.
; Uses BX as band index and SI as table pointer.

draw_road_bands:
        mov  bx, ROAD_START_BAND

road_band_loop:
        cmp  bx, ROAD_NUM_BANDS
        jge  road_band_done

        ; y = ROAD_HORIZON_Y + band*4
        mov  ax, bx
        shl  ax, 1
        shl  ax, 1
        add  al, ROAD_HORIZON_Y
        mov  road_y, al

        ; center = road_center_table[band]
        mov  si, offset road_center_table
        add  si, bx
        mov  al, [si]
        mov  road_center, al

        ; halfw = road_halfwidth_table[band]
        mov  si, offset road_halfwidth_table
        add  si, bx
        mov  al, [si]
        mov  road_halfw, al
        ...
        mov  al, road_width
        mov  rect_w, al
        mov  rect_h, ROAD_BAND_H
        mov  rect_c, COL_ROAD
        call fill_rect
        call blit_wait

        ; optional dashed stripe
        mov  si, offset road_stripe_table
        add  si, bx
        mov  al, [si]
        or   al, al
        jz   no_stripe
...
; ---------------------------------------------------------------------------
; SUBROUTINE: draw_road_scene
; ---------------------------------------------------------------------------
; Draw sky, ground, then the road bands.

draw_road_scene:
        ; sky
        mov  rect_y, 0
        mov  rect_h, ROAD_HORIZON_Y
        mov  rect_c, COL_SKY
        call draw_fullwidth_rect

        ; ground / verge
        mov  rect_y, ROAD_HORIZON_Y
        mov  rect_h, 0C8h-ROAD_HORIZON_Y
        mov  rect_c, COL_GRASS
        call draw_fullwidth_rect

        call draw_road_bands
        ret
